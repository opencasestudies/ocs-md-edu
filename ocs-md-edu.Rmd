...
data[[1]]
```
Right now we can see that many of the column names have spaces, which makes them harder to work with in R.  We can replace the spaces with . using the `make.names() function`
```{r}
names(data[[1]])
make.names(names(data[[1]]))
```
We can do this for each of the 5 data sets using a for loop:
```{r}
for (i in 1:5) {
