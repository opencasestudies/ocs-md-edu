...
also the slope for the effect of `non.grad.rate.log`. It means that to 1 unit decrease(1%) in the `non.grad.rate.log`, there is `r round((-coef(summary(m2))[2,1]),2)` increase in `pro`. Then, there’s a standard error associated with this slope, and a t- value, which is simply the estimate divided by the standard error.
