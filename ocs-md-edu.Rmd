...
We notice big differences in the values for these two graduation rates -- the 3-year rate is generally quite low while the 4-year rate is generally in the 80-100% range we might expect for a graduation rate.  Since high school students typically graduate after four years, it makes more sense to use the 4-year adjusted cohort as our measure of a school's graduation rate.
...
ggarrange(p3, p4, p5, nrow = 2, ncol = 2, 
          labels = c('df_ALG1', 'df_ALG2', 'df_ELA'), 
...
## Predictor Variable
```{r, fig.width= 10, fig.height=8}
p6 <- distplot(df_ALG1, df_ALG1$grad.rate)
p7 <- distplot(df_ALG2, df_ALG2$farms)
p8 <- distplot(df_ALG1, df_ALG1$withdraw)
p9 <- distplot(df_ALG2, df_ALG2$exp)
...
```{r, fig.height= 10}
relaplot1 <- function(variable){
  
  p <- ggplot(df_ALG1, aes(x = variable, y = pro))+    
...
relaplot2 <- function(variable){

  p <- ggplot(data = df_ALG1, aes(x = variable , y = pro,group = county))+    
...
p10 <- relaplot1(df_ALG1$farms)
p11 <- relaplot2(df_ALG1$farms)
p12 <- relaplot1(df_ALG1$grad.rate)
p13 <- relaplot2(df_ALG1$grad.rate)

ggarrange(p10,p11,p12,p13, ncol = 2, nrow = 2, widths = c(3,6))
```
...
In the random effect part, it measures how much variability in the dependent variable `pro` there is due to the random effect `county`. The variance for `county` is 154.1 - how much variance there is between counties in Algebra 1 achievement. The residual is 201.1 - stands for the variability that???s not due to `county`. To partition the variance, we take the variance component of the intercept and divide it by the sum of the residual variance and the intercept variance. The result means county-level factors explain `r paste(round(100*154.1/(154.1+201.1), 2), "%", sep="")` of the variance. 

The fixed effect part provides model intercepts that are not particularly meaningful in MLM. It is the average for all schools in our dataset.
